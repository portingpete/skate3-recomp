...
bool IsRepeatedVfsEntryNotFoundDetailLog(std::string_view text) {
  return text.find("VFS: entry not found for") != std::string_view::npos &&
         ...
         text.find("suppressed") == std::string_view::npos &&
         text.find("suppressing") == std::string_view::npos;
}

bool IsRepeatedVfsEntryNotFoundSuppressionLog(std::string_view text) {
  return text.find("VFS: entry not found") != std::string_view::npos &&
         text.find("eyereflection.ddx") != std::string_view::npos &&
         text.find("suppressing repeated misses") != std::string_view::npos;
}
...
bool IsXctdCompressionInformationCompatibilityLog(std::string_view text) {
  const bool has_path = text.find("data\\xctd.bin") != std::string_view::npos ||
                        text.find("data/xctd.bin") != std::string_view::npos;
  return text.find("XFileXctdCompressionInformation") != std::string_view::npos && has_path &&
         text.find("status=0xc000000d") != std::string_view::npos;
}
...
TEST_CASE("NtReadFile explains unreadable zero-byte shader buffers",
          "[runtime][kernel][xboxkrnl][io]") {
  const auto root =
      std::filesystem::temp_directory_path() /
      ("rex_ntreadfile_zero_byte_shader_unreadable_" +
       std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
  std::error_code cleanup_error;
  std::filesystem::remove_all(root, cleanup_error);
  std::filesystem::create_directories(root / "shaders");
  ...
  rex::system::X_IO_STATUS_BLOCK read_iosb{};
  constexpr u32 kUnmappedBufferGuest = 0x7F000000;
  CHECK(rex::kernel::xboxkrnl::NtReadFile_entry(
            static_cast<u32>(handle), 0, mapped_void(nullptr), mapped_void(nullptr),
            ppc_ptr_t<rex::system::X_IO_STATUS_BLOCK>(&read_iosb, 0x40004000),
            mapped_void(nullptr, kUnmappedBufferGuest), 0, mapped_u64(nullptr)) ==
        X_STATUS_SUCCESS);
  CHECK(static_cast<u32>(read_iosb.status) == X_STATUS_SUCCESS);
  CHECK(static_cast<u32>(read_iosb.information) == 0);
  ...
  const auto diagnostic_count =
      std::count_if(krnl_entries.begin(), krnl_entries.end(), [](const rex::LogEntry& entry) {
        return entry.level == spdlog::level::debug &&
               IsUnreadableZeroByteShaderDiagnostic(entry.text);
      });
  CHECK(diagnostic_count == 1);
...
TEST_CASE("XFileXctdCompressionInformation returns invalid parameter without error noise",
          "[runtime][kernel][xboxkrnl][io]") {
  const auto root =
      std::filesystem::temp_directory_path() /
      ("rex_file_xctd_info_log_" +
       std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
  std::error_code cleanup_error;
  std::filesystem::remove_all(root, cleanup_error);
  std::filesystem::create_directories(root / "data");
  {
    std::ofstream file(root / "data" / "xctd.bin", std::ios::binary);
    REQUIRE(file.good());
    file << "xctd-info";
  }
  ...
  auto* memory = runtime.kernel_state()->memory();
  const u32 path_guest = StoreAnsiString(memory, "game:\\data\\xctd.bin");
  ...
  rex::system::X_IO_STATUS_BLOCK query_iosb{};
  rex::system::X_FILE_XCTD_COMPRESSION_INFORMATION xctd_info{};
  xctd_info.unknown = 0xFFFFFFFFu;
  CHECK(rex::kernel::xboxkrnl::NtQueryInformationFile_entry(
            static_cast<u32>(handle),
            ppc_ptr_t<rex::system::X_IO_STATUS_BLOCK>(&query_iosb, 0x40004000),
            mapped_void(&xctd_info, 0x40005000), sizeof(xctd_info),
            rex::system::XFileXctdCompressionInformation) == X_STATUS_INVALID_PARAMETER);
  CHECK(static_cast<u32>(query_iosb.status) == X_STATUS_INVALID_PARAMETER);
  CHECK(static_cast<u32>(query_iosb.information) == 0);
  CHECK(static_cast<u32>(xctd_info.unknown) == 0);
  ...
  const auto error_count =
      std::count_if(krnl_entries.begin(), krnl_entries.end(), [](const rex::LogEntry& entry) {
        return entry.level >= spdlog::level::warn &&
               entry.text.find("XFileXctdCompressionInformation") != std::string_view::npos;
      });
  const auto debug_count =
      std::count_if(krnl_entries.begin(), krnl_entries.end(), [](const rex::LogEntry& entry) {
        return entry.level == spdlog::level::debug &&
               IsXctdCompressionInformationCompatibilityLog(entry.text);
      });
  CHECK(error_count == 0);
  CHECK(debug_count == 1);
...
TEST_CASE("Repeated VFS entry misses are coalesced",
  ...
  const auto root =
      std::filesystem::temp_directory_path() /
      ("rex_repeated_vfs_miss_log_" +
       std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
  std::error_code cleanup_error;
  std::filesystem::remove_all(root, cleanup_error);
  std::filesystem::create_directories(root / "Data" / "Textures" / "Effects");
  ...
  for (size_t i = 0; i < 8; ++i) {
    CHECK(fs->ResolvePath("game:\\Data\\Textures\\Effects\\eyereflection.ddx") == nullptr);
  }
  CHECK(fs->ResolvePath("game:\\Data\\Textures\\Effects\\reflection.ddx") == nullptr);

  std::vector<rex::LogEntry> fs_entries;
  fs_sink->CopyEntries(fs_entries);
  rex::RemoveSink(rex::log::fs(), fs_sink);

  CHECK(std::count_if(fs_entries.begin(), fs_entries.end(), [](const rex::LogEntry& entry) {
    return IsRepeatedVfsEntryNotFoundDetailLog(entry.text);
  }) == 4);
  CHECK(std::count_if(fs_entries.begin(), fs_entries.end(), [](const rex::LogEntry& entry) {
    return IsRepeatedVfsEntryNotFoundSuppressionLog(entry.text);
  }) == 1);
  CHECK(std::any_of(fs_entries.begin(), fs_entries.end(), [](const rex::LogEntry& entry) {
    return entry.text.find("reflection.ddx") != std::string_view::npos &&
           entry.text.find("suppressed") == std::string_view::npos;
  }));
